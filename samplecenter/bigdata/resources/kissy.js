...
(function(S, loader, utils) {
    ...
    S.mix(utils, {
        ...
        removePostfix:function (path) {
            return path.replace(new RegExp("(-min)?\.js[^/]*$","i"), "");
        },
        /**
         * 路径正则化，不能是相对地址
         * 相对地址则转换成相对页面的绝对地址
...
(function(S, loader, data) {
    ...
    mix(loader, {
        ...
        // ->
        // http://xx.com/y/
        __pagePath:location.href.replace(location.hash, "").replace(new RegExp("[^/]*$","i"), ""),
...
(function (S, loader, utils) {
     ...
     */
        // notice: timestamp
    var baseReg = new RegExp("^(.*)(seed|kissy)(-aio)?(-min)?\.js[^/]*","i"),
