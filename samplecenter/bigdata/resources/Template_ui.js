...
fastDev.define("fastDev.Core.Template", {
	...
	pushHtml : function(fnContent, html) {
		fnContent.push("html.push('" + html.replace(this.captureParam, "' + params.$1 + '").replace(this.captureData, "' + item[\"$1\"] + '")  + "'); \n");
	},
	/**
	 * 解析模板关键字 
	 * @param {Array} fnContent 模板函数内容数组
	 ...
	 */
	buildDynamicTemplate : function(name, params, data){
		var dynamic = this.getDynamic(name);
		return dynamic(params, data);
	},
	/**
	 * 根据动态模板名称获取动态模板 
	 * @param {String} name 动态模板名称
