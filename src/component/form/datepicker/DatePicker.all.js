...
fastDev.define("fastDev.Ui.DatePicker", {
    ...
    isDisabled: function (year, month, date, hours, minutes, seconds) {
        ...
        if (!disabled) {
            ...
            // 先判断排除
            idx = 0;
            while (expr = excludes[idx++]) {
                if (this.contains(expr, time, true)) {
                    disabled = true;
                    break;
                }
            }
            ...
            if (!disabled) {
                idx = 0;
                while (expr = includes[idx++]) {
                    // 如果含include声明，则拟定待判断值是需要disabled的
                    disabled = true;
                    if (this.contains(expr, time)) {
                        disabled = false;
                        break;
                    }
                }
